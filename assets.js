...
router.get("/:id", async (req, res) => {
    try {
        ...
        // Fetch asset by ID for this user
        const result = await pool.query(
...
router.post("/save", upload.single("photo"), async (req, res) => {
    ...
    try {
        ...
        const photo_url = req.file ? req.file.path : null; // Cloudinary URL automatically stored here

        const result = await pool.query(
            `INSERT INTO assets 
        (name, category, manufacturer, model, serial_number, location, purchase_date, warranty_expiry, status, notes, user_id, photo_url, created_at, updated_at)
